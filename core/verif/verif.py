...
class TestResults:
    ...
    def __init__(self):
        ...
        # Allow either a callable or a function name from testspec.json
        # this automatically resolves the string to the actual function object, 
        # and raises an error if the function name is not found or if the resolved object is not callable

        # the list of allowed test functions is automatically generated by looking for functions in the global scope 
        # that start with "test_" and have at least the signature (results, model_output_map, empirical_data_path, testname)
        # if isinstance(test_function, str):
        expected_params = ["results", "model_output_map", "empirical_data_path", "testname"]
        available_test_functions = {}

        # find all functions in this file starting with "test_"
        for function_name, function_obj in globals().items():
            if not (function_name.startswith("test_") and callable(function_obj)):
                continue

            signature = inspect.signature(function_obj)
            parameter_names = list(signature.parameters.keys())

            # check that the potential function has at least the expected parameters in the correct order (it can have additional optional parameters after those)
            if all(param in parameter_names for param in expected_params):
                available_test_functions[function_name] = function_obj

        self.test_functions = available_test_functions
...
def launch_empirical_test(results, memspecs, workload, empirical_data_path, test_function, testname):
    ...
    if isinstance(test_function, str):
        ...
        resolved = results.test_functions.get(test_function)
        if resolved is None:
            valid_names = ", ".join(sorted(results.test_functions.keys()))
            raise ValueError(
                f"Unknown test_function '{test_function}'. Valid options: {valid_names}"
            )
        test_function = resolved

    if not callable(test_function):
        raise TypeError(
            f"test_function must be callable or function-name string, got {type(test_function).__name__}"
        )
...
def run_all_tests(empirical_data_path=None, plot=False):
    ...
    with open(test_inputs_dir / "testlist.txt", 'r') as f:
        testnames = [line.strip() for line in f if line.strip()]
    ...
    # run each test listed in testlist.txt
    for testname in testnames:
        workload_file, empirical_data_path = get_workload_and_empirical(testname)
        if not workload_file or not empirical_data_path:
            print(f"[WARN] Missing workload or empirical data for test {testname} - skipping")
        # read testspec.json to get the test_function and testname:
        try:
            path = Path(os.path.dirname(__file__)) / "test_inputs" / testname / "testspec.json"
        ...
        launch_empirical_test(results, memspec_paths, workload_file, empirical_data_path, raw["test_function"], raw["testname"])
