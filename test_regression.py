...
def verify_model_output_exists():
    ...
    if baseline_path.exists():
        ...
        if 0.01 <= total_power <= 100.0:
            print(f"\n✓ Total power in reasonable range: {total_power:.4f} W")
            return True
        else:
            print(f"\n⚠ Total power outside expected range: {total_power:.4f} W")
            return False
    else:
    ...
    for key in current_output.keys():
        ...
        if baseline_val is None:
            ...
            if rel_diff > tolerance:
                ...
                print(f"  ✗ {key}: {baseline_val:.6f} → {current_val:.6f} W")
            else:
                print(f"  ✓ {key}: {current_val:.6f} W")
    
    if differences:
        print(f"\n⚠️  {len(differences)} difference(s) detected:")
        for diff in differences:
            print(f"    • {diff}")
        return False
    else:
        print("\n✅ All model outputs match baseline")
        return True
  ...
  python test_regression.py --compare
    ...
    if args.compare:
        ...
        if not compare_model_outputs():
            print("\n❌ Regression check failed: Model outputs have changed")
            sys.exit(1)
        else:
            print("\n✅ Regression check passed: Model outputs unchanged")
            sys.exit(0)
    ...
    if not verify_model_output_exists():
        print("\n⚠️  Warning: Baseline verification issues detected")
